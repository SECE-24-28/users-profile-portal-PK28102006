...
const ADD_STUDENT = gql`
  mutation AddStudent($name: String!, $email: String!, $age: Int!, $course: String!, $profileUrl: String) {
    addStudent(name: $name, email: $email, age: $age, course: $course, profileUrl: $profileUrl) {
      id
      name
    }
  }
...
export default function AddStudentPage() {
  ...
  const [addStudent, { loading: mutationLoading }] = useMutation(ADD_STUDENT, {
    onCompleted: () => {
      router.push('/students');
      router.refresh();
    },
    onError: (err) => {
      setErrorMsg(err.message || 'Failed to create student. Check inputs.');
    },
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name || !email || !age || !course) {
      setErrorMsg('All fields except profile picture are required.');
      return;
    }
    ...
    <DashboardLayout>
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="bg-white border border-slate-200 rounded-lg p-6 sm:p-8 shadow-sm">
          <h2 className="text-xl font-bold text-slate-900 mb-6">Enter Student Details</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="text-xs text-slate-550">
                      {file ? (
                        ...
                        <span>Upload photo (PNG, JPG)</span>
                      )}
                    </p>
                  </div>
            ...
            <div className="flex items-center justify-end gap-4 border-t border-slate-200 pt-6">
              ...
              >
                {uploading ? 'Uploading Photo...' : mutationLoading ? 'Saving Student...' : 'Create Profile'}
              </button>
            </div>
          </form>
        </div>
